...
class Task:
    def __init__(self, model_name, data_name, cv_runs, params_dict, logger, portion=100, save_name=''):
        ...
        single_mask = []
        sig_set = set()
        for x in one_label_ids:
            sig_set.add(x)
        if params_dict['only_single']:
            ...
            single_mask = [ 0.0 for i in range(len(labels_train))]
        single_mask = np.array(single_mask).astype(np.float)
        print('single mask multi rate:',np.sum(single_mask)/single_mask.shape[0])
        ...
        self.train_set = list(
            zip(words_train, textlen_train, mentions_train, mentionlen_train, positions_train, labels_train, train_id, single_mask))
        self.valid_set = list(
    ...
    def refit(self):
        ...
        for i in range(self.cv_runs):
            ...
            for epoch in range(1, self.num_epochs + 1):
                ...
                epoch_ids, epoch_losses = self.model.evaluate(sess, train_batches,epoch)

                preds, maxtype, h_output = self.model.predict(sess, self.test_set)
                acc, macro, micro = self.get_scores(maxtype)
                vapreds, _ , val_h = self.model.predict(sess, self.valid_set)
                ...
                if cmp >= max_va_acc:
                    ...
                    if 'is_exp' in self.hparams.keys() and self.hparams['is_exp']:
                        np.save(f"./exp/visual/{self.data_name}_{epoch}_Tacc_{acc}",h_output)
                        np.save(f"./exp/visual/{self.data_name}_{epoch}_Tacc_{acc}_true",self.labels_test)
